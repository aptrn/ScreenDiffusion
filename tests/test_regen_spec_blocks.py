...
def test_the_five_generated_blocks_are_all_of_them():
    """One entry per spec block generated from a committed result: 7.2's marginal
    table, 8.1's detectors, 8.2's primitives, 8.8's selective path, 7.4's deploy
    hardware. A sixth block added without an entry here is a hand-copy waiting."""
    assert set(regen.BLOCKS) == {
        "MEASURED TABLE", "DETECTOR TABLE", "PRIMITIVE DECISION",
        "SELECTIVE PATH", "DEPLOY HARDWARE",
    }
